...
static inline float *dilation_change(int M, int C, int K, int dilation, float* m_mat){
    float *mat = (float *) malloc(K * K * M * C *sizeof(float));
    int b_K = (K - 1)/dilation + 1;
    ...
    for(int x = 0; x < M; x++)
        for(int y = 0; y < C; y++)
              for(int i = 0; i < K; i++)
                  for(int j = 0; j < K; j++)
                      mat[x * C * K * K + y * K * K + i * K + j] = 0.0;
    for(int x = 0; x < M; x++)
        for(int y = 0; y < C; y++)
              for(int i = 0; i < b_K; i++)
                  for(int j = 0; j < b_K; j++)
                      mat[x * C * K * K + y * K * K + i * dilation * K + j * dilation] = m_mat[x * C * b_K * b_K + y * b_K * b_K + i * b_K + j];
  return mat;
}
static inline float *rand_1_mat(int f, int c, int k) {
  ...
  for(int x = 0; x < f; x++)
    for(int y = 0; y < c; y++)
      for(int i = 0; i < k; i++){
        for(int j = 0; j < k; j++){
          mat[x * c * k * k + y * k * k + i * k + j] = 1.0;//(float)(rand() % 100);
        }
      }
  return mat;
}
static inline float *rand_2_mat(int w_channel, int h, int w) {
  ...
  for(int k = 0; k < w_channel; k++)
      for(int i = 0; i < h; i++){
         for(int j = 0; j < w; j++){
            mat[k * h * w + i * w + j] = 1.0;//(float)(rand() % 100);
         }
      }
  return mat;
}
